...
- (void)prepareForReuse {
    self.zoomScale = 0.0f;
    self.minimumZoomScale = 0.0f;
    self.maximumZoomScale = 0.0f;
...
- (void)setPhoto:(id<MWPhoto>)photo {
    ...
    _photo = photo;
    if (_recommandView.superview) {
        [_recommandView removeFromSuperview];
    }
    if (!_tapView.superview) {
        [self addSubview:_tapView];
    }
    if (!_photoImageView.superview) {
        [self addSubview:_photoImageView];
    }
    if ([photo isKindOfClass:[MWPhoto class]]) {
    ...
    else if ([photo isKindOfClass:[MWGroupPhoto class]]) {
        ...
        if (_photo) {
            _recommandView = [[MWRecmandView alloc]initWithFrame:self.bounds];
            _recommandView.groupPhoto = _photo;
            _recommandView.photoBrowser = _photoBrowser;
            if (!_recommandView.superview) {
